...
def cost_to_go(test_state,goal_state):
	return  np.sqrt((test_state[0] - goal_state[0])**2 + (test_state[1] - goal_state[1])**2 ) + 0.1*(min(np.absolute((test_state[2] - goal_state[2])*np.pi/180),2*np.pi - np.absolute((test_state[2] - goal_state[2])*np.pi/180)) )
...
class Game:
	def __init__(self):
		...
		self.car_image = pygame.transform.scale(self.car_image,(24*2,24*2))
	...
	def simulate_forward(self, time, init_state, control_input):
			...
			velocity_left = control_input[0]
	   

			velocity_right = control_input[1]
			...
			for t in time_steps:
				...
				x += self.r*(velocity_right + velocity_left)*np.cos(theta)*0.1/2
				y += self.r*(velocity_right + velocity_left)*np.sin(theta)*0.1/2

				theta += self.r*(velocity_right - velocity_left)*0.1/self.L
	...
	def A_star(self,init_state,goal_state):
		...
		velocity_left = np.arange(-5.0,5.1,1)

		velocity_right = np.arange(-5.0,5.1,1)

		control_inputs = generate_controls_matrix(velocity_left, velocity_right)
	...
	def run(self):
		...
		for control in reversed(self.controls):
			...
			velocity_left = control[0]
	   

			velocity_right = control[1]
			...
			for t in time_steps:
				...
				self.x += self.r*(velocity_right + velocity_left)*np.cos(self.theta)*0.1/2
				self.y += self.r*(velocity_right + velocity_left)*np.sin(self.theta)*0.1/2

				self.theta += self.r*(velocity_right - velocity_left)*0.1/self.L
			...
			# Drawing
				...
				BLACK = (0, 0, 0)
				#pygame.draw.rect(self.screen, BLACK, pygame.Rect(1000, 360, 250, 150))
